    ...
    const columns = [
        {
            key: 'image',
            label: 'Image',
            render: (row) => (row.imageUrls && row.imageUrls.length > 0) ? (
                <img src={row.imageUrls[0]} alt={row.name}
        ...
        {key: 'sku', label: 'SKU'},
        {
            key: 'category',
            label: 'Category',
            render: (row) => row.category?.name || <span style={{color: '#94a3b8'}}>Uncategorized</span>
        },
